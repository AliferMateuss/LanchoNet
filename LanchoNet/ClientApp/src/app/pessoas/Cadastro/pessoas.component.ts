...
export class PessoasComponent implements OnInit {
  ...
  constructor(private http: HttpClient, @Inject('BASE_URL') private baseUrl: string, private fb: FormBuilder,
    private cdr: ChangeDetectorRef, private validator: GenericValidator, private modalService: BsModalService, private route: ActivatedRoute, private router: Router) { }
  ...
  ngOnInit(): void {
    ...
    if (id) {
      const params = new HttpParams()
        .set("id", id)
      this.http.get<Pessoa>(this.baseUrl + 'api/Pessoas/RetornaPessoaPorId', { params }).subscribe(data => {
        console.log(data);
        this.pessoa = data;
        if (data.dataNascimento != null) {
          this.pessoa.dataNascimento = data.dataNascimento.split('T')[0];
        }
  ...
  salvarPessoa() {
    ...
    if (this.formPessoa.valid) {
      ...
      if (this.ehAlteracao) {
        ...
        this.http.post(this.baseUrl + 'api/Pessoas/Atualizar', this.pessoa).subscribe(data => {
          this.limpaPessoa();
  ...
  alterarEndereco(endereco: Endereco) {
    ...
    if (this.ehAlteracao) {
      ...
      if (endereco.idCidade != null) {
        const params = new HttpParams().set("id", endereco.idCidade)
        this.http.get<any>(this.baseUrl + 'api/Cidade/RecuperaCidadePorId', { params }).subscribe(data => {
          var array = [];
          array.push(data);
          this.cidades = array;
          this.cdr.detectChanges(); 
  ...
  get cpf() { return this.formPessoa.get('cpf')!; }
